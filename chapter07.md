...
![小图标](images/小图标.png '小图标')

可以使用i或span标签来配合使用，具体如下：:smile:
...
注意：这里div中并没有实现 class="dropdown"，通过源码分析知道嵌套本身已经有定位就不需要再设置。而右边的圆角只要多加一个 class="dropdown-toggle"即可。
...
这个下拉菜单其实和第二个知识点一样，只不过，这个是在群组里，不需要div声明class="dropdown"。
